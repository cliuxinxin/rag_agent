            ...
            with st.spinner("AI 正在重构文案，适配推特生态..."):
                try:
                    ...
                    if "Thread" in twitter_mode:
                        ...
                        【内容与排版要求】：
                        1. 黄金首推（1/N）：必须是极具吸引力的 Hook，一句话点破痛点或抛出反常识结论。
                        2. 充实单推信息量：每条推文字数控制在 140 字以内。不要切得太碎！每一推都要包含完整且高密度的逻辑、数据或金句。
                        3. 🚫 严禁使用 Markdown（极其重要）：X 平台不支持 Markdown！绝对不要出现 `**加粗**`、`# 标题`、`- 列表` 等符号！
                        4. 🎨 巧用 Emoji 排版：既然不能加粗，请务必使用相关的 Emoji（如 💡, 📊, ⚠️, 📌, 👉 等）来代替标记符号，划分重点和层次，增强视觉冲击力。
                        ...
                        【长推文排版要求】：
                        1. 🚫 严禁使用 Markdown（极其重要）：X 平台不支持 Markdown！绝对不要出现 `**加粗**`、`# 标题` 等符号，这会让排版显得极不专业！
                        2. 🎨 巧用 Emoji 划分结构：用 Emoji（如 🔹, 💡, 🚀, 📌 等）来替代 Markdown 的强调效果，作为段落标题或列表的引导。
                        3. 结构紧凑：第一段必须是极具吸引力的 Hook。多用短句，段落之间留出空行，确保手机端向下滚屏阅读极度丝滑。
                        4. 提炼干货：提取原文最核心的金句和数据，去掉啰嗦的过渡语。
                    ...
                    # 终极清洗：防止 AI 偶尔失控还是输出了 **，用代码强制剔除掉
                    thread_result = thread_result.replace("**", "").replace("__", "").replace("# ", "")
