...
# [新增] 角度生成 Prompt
def get_angle_proposal_prompt(raw_content: str, instruction: str) -> str:
    return f"""
你是一名资深的主编。我们在策划一篇新文章。
...
请根据素材，构思 3 个截然不同的【写作方向/切入角度】供用户选择。

要求：
1. **方向 A (深度分析)**：侧重逻辑、原理和深层原因。
2. **方向 B (行业故事)**：侧重时间线、人物冲突和细节描写。
3. **方向 C (犀利观点)**：侧重提炼核心观点，具有争议性或前瞻性。

请严格输出 JSON 格式（List of Dicts），无需其他文字：
[
    {{
        "id": "A",
        "label": "深度分析：[具体标题]",
        "description": "这个方向将重点讨论...",
        "pros": "适合..."
    }},
    {{
        "id": "B",
...
# [新增] 标题生成 Prompt (依赖选定的角度)
def get_title_gen_prompt(raw_content: str, selected_angle_desc: str) -> str:
    return f"""
【任务】基于素材和选定的切入角度，拟定一个极具吸引力的主标题。
