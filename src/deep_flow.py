...
def writer_node(state: AgentState) -> dict:
    ...
    2. **架构的系统视角**：解释模型组件（如 HOPE, Titans）。**使用文字描述架构或逻辑流即可，不要生成 Mermaid 图表代码**。
    ...
    2. **系统架构与选型 (The Stack)**：
       - **硬件层**：主板、CPU、机箱、存储的分层设计。**使用文字描述硬件连接或数据流，不要生成 Mermaid 图表代码**。
    ...
    【重要控制指令】(CRITICAL)
    1. **完整性优先**：必须写完所有章节。如果内容过多，请适当精简"直觉层"的废话，确保"核心数据"和"数学内核"的完整性。
    2. **纯文本输出**：不要生成 Mermaid 图表代码。使用文字描述架构或逻辑流即可。
    3. **数学公式**：使用 LaTeX ($$...$$)。
